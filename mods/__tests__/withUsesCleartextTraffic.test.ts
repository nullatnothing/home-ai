...
describe('withUsesCleartextTraffic', () => {
  ...
  it('writes the network security config', async () => {
    ...
    const result = await withUsesCleartextTraffic(config, { ips: ['10.0.2.2'] });

    expect(fs.mkdirSync).toHaveBeenCalledWith(path.join('/app', 'android', 'app', 'src', 'main', 'res', 'xml'), { recursive: true });
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      path.join('/app', 'android', 'app', 'src', 'main', 'res', 'xml', 'network_security_config.xml'),
      expect.stringContaining('<domain includeSubdomains="true">10.0.2.2</domain>'),
      'utf8',
    );

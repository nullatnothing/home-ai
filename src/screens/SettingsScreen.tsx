...
export function SettingsScreen({ appState, setAppState, strings }: Props) {
  ...
  const canSave = Boolean(normalizedServerUrl) && normalizedServerUrl !== appState.serverUrl;
  ...
  const saveServerUrl = () => {
    ...
    setAppState((current) => ({ ...current, serverUrl: nextUrl, dialog: { title: strings.serverUpdatedTitle, message: `${strings.serverUpdatedMessage} ${nextUrl}`, confirmText: strings.ok } }));
  };
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setAppState((current) => ({ ...current, serverUrl: normalized, availableModels: result.models?.map((model) => model.name) ?? current.availableModels, lastConnectionState: 'success', lastConnectionMessage: strings.connectionSuccessful, dialog: { title: strings.connectionSuccessful, message: `${strings.connectedTo} ${normalized}`, confirmText: strings.ok } }));
    } catch (error: any) {
      setAppState((current) => ({ ...current, serverUrl: normalized, lastConnectionState: 'error', lastConnectionMessage: error?.message ?? strings.unknownError, dialog: { title: strings.connectionFailed, message: error?.message ?? strings.unknownError, confirmText: strings.ok } }));
    } finally {
      setTesting(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.safeArea} contentContainerStyle={styles.settingsScrollContent}>
      ...
      <View style={styles.settingsCard}>
        <Text style={styles.settingsSectionTitle}>{strings.ollamaServerUrl}</Text>
        <TextInput value={serverUrl} onChangeText={setServerUrl} style={styles.renameInput} placeholder={strings.ollamaServerUrl} />
        <View style={styles.buttonRow}>
          <Pressable disabled={!canSave} style={[styles.primaryButton, styles.fullWidth, !canSave && styles.primaryButtonDisabled]} onPress={saveServerUrl}><Text style={[styles.primaryButtonText, !canSave && styles.primaryButtonTextDisabled]}>{strings.saveServerUrl}</Text></Pressable>
      ...
      <View style={styles.settingsCard}>
        ...
        <View style={styles.languageSelector}>
          {LANGUAGE_OPTIONS.map((code) => (
            <Pressable key={code} onPress={() => setAppState((current) => ({ ...current, language: code }))} style={[styles.languageChip, appState.language === code && styles.languageChipSelected]}>
              <Text style={[styles.languageChipText, appState.language === code && styles.languageChipTextSelected]}>{code.toUpperCase()}</Text>
            </Pressable>
          ))}
        </View>
      </View>
